...
const EndingsSystem = (() => {
  ...
  const FOUNDER_NORMAL_ENDINGS = [
    {
      id: 'normal_burnout', emoji: '💀', title: '번아웃 (Series A)',
      subtitle: '멘탈 붕괴',
      description: 'Series A 레이스는 Seed보다 훨씬 길고 가혹했다. 결국 사람이 무너졌다.',
      condition: (s) => s.stats.mental.value <= 0,
      priority: 100,
    },
    {
      id: 'normal_runway_end', emoji: '💸', title: '런웨이 종료 (Series A)',
      subtitle: 'Series A 직전에',
      description: '"거의 다 왔는데..." 가장 잔인한 결말 중 하나다.',
      condition: (s) => s.stats.runway.value <= 0,
      priority: 99,
    },
    {
      id: 'normal_bad_terms', emoji: '🥴', title: '독소 조항 서명',
      subtitle: '빠른 클로징의 대가',
      description: '50억은 들어왔다. 하지만 drag-along 60%에, full ratchet까지. 다음 라운드에서 당신은 남의 회사 대표가 된다.',
      condition: (s) => s.flags.includes('signed_bad_normal') && !s.flags.includes('series_a_closed'),
      priority: 85,
    },
    {
    ...
    {
      id: 'normal_bootstrap', emoji: '🌿', title: '부트스트랩 마스터',
      subtitle: '외부 자금 없이 간다',
      description: '투자를 거절했다. 아니, 조건이 나빠서 못 받았다. 이제 매출로 버티는 길을 선택했다.\n\n더디지만, 주도권을 지키는 선택이다.',
  ...
  const FOUNDER_HARD_ENDINGS = [
    {
      ...
      subtitle: '시차, 언어, 문화 — 모든 것이 적이었다',
      description: '뉴욕 로드쇼, 싱가포르 미팅, 국내 LP 설명회... 결국 사람이 버티지 못했다.',
      condition: (s) => s.stats.mental.value <= 0,
      priority: 100,
    },
    {
      id: 'hard_runway_end', emoji: '💸', title: '런웨이 종료 (Series B)',
      subtitle: '300억 앞에서 무너지다',
      description: '글로벌 DD에 3개월. Cayman Flip에 2개월. 그리고... 런웨이가 끝났다.',
      condition: (s) => s.stats.runway.value <= 0,
      priority: 99,
    },
    {
      id: 'hard_deal_collapse', emoji: '💔', title: '딜 붕괴',
      subtitle: '마지막 순간에',
      description: 'IC 직전까지 갔다. 모든 서류가 준비됐다. 그리고 해외 LP가 한국 시장 리스크를 이유로 거부했다.',
      condition: (s) => s.flags.includes('deal_collapsed'),
      priority: 85,
    },
    {
      id: 'hard_poison_terms', emoji: '🥴', title: '독소 조항 (Series B)',
      subtitle: '글로벌 독소 조항',
      description: '해외 VC의 drag-along 60%에 1.5x liquidation preference. 한국 법무팀도 검토 못 했다.',
      condition: (s) => s.flags.includes('signed_bad_hard'),
      priority: 80,
    },
    {
      id: 'hard_global_round', emoji: '🌍', title: '글로벌 라운드 클로저',
      subtitle: '한국을 넘어 세계로',
    ...
    },
    {
      ...
      description: 'VC 딜은 안 됐지만, 대기업 CVC와 전략적 파트너십을 맺었다. 다른 길로 글로벌에 도달했다.',
    ...
    },
    {
      ...
      description: 'Series B 글로벌 라운드는 아직이다. 하지만 로드쇼를 통해 쌓인 네트워크는 언젠가 빛날 것이다.',
      condition: () => true,
      priority: 0,
    },
  ];
